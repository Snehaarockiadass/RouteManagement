window.sessionStorage;
    ...
    var pagerName;
    var routeCount = sessionStorage.getItem("routeCount");
    
    var pager = new Pager('route-info', PerPagesize);
    pager.init();
    pager.showPageNav('pager', 'pageNavigationPosition');
...
var pagerName = 'pager';
        ...
        var showingPage = document.getElementById('showPage');
        //showingPage.innerHTML = "<label>Showing " + noOfPages+ " results out of " + records+"</label>";

 

        document.getElementById('routeDisplayCount').innerText ='Display : '+ $('#route-info > tr:not([style*="display: none"])').length+" out of " +rowCounter;
      ...
      this.createPager = function (startEle, EndEle) {
        ...
        if (EndEle <= this.pages && startEle > 0) {
         ...
         var element = document.getElementById("pageNavigationPosition");
